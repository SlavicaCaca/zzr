---
layout: about-page
title: "Sve kategorije"
permalink: /kategorije/
---

<div class="category-grid">
  <div class="category-card">
    <h2><a href="{{ '/kategorije/feminizam-i-rod/' | relative_url }}">Feminizam i rod</a></h2>
    <p>Feminizam, rodna ravnopravnost, ženska prava</p>
  </div>
  
  <div class="category-card">
    <h2><a href="{{ '/kategorije/kvir-aktivizam/' | relative_url }}">Kvir aktivizam</a></h2>
    <p>LGBTQ+ zajednica, kvir aktivizam</p>
  </div>
  
  <div class="category-card">
    <h2><a href="{{ '/kategorije/nasilje/' | relative_url }}">Nasilje</a></h2>
    <p>Rodno nasilje, sistemski problemi, prevencija</p>
  </div>
  
  <div class="category-card">
    <h2><a href="{{ '/kategorije/drustvo-kultura/' | relative_url }}">Društvo i kultura</a></h2>
    <p>Mediji, pop kultura, obrazovanje, politika</p>
  </div>
  
  <div class="category-card">
    <h2><a href="{{ '/kategorije/zdravlje-telo/' | relative_url }}">Zdravlje i telo</a></h2>
    <p>Mentalno zdravlje, reproduktivna prava, ginekologija</p>
  </div>
  
  <div class="category-card">
    <h2><a href="{{ '/kategorije/feministicka-teorija/' | relative_url }}">Feministička teorija</a></h2>
    <p>Feministička teorija, kritička analiza, filozofija</p>
  </div>
  
  <div class="category-card">
    <h2><a href="{{ '/kategorije/prakticne-teme/' | relative_url }}">Praktične teme</a></h2>
    <p>Priručnici, resursi, radionice, iskustva</p>
  </div>
</div>
